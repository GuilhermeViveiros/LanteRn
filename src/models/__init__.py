...
def load_model(model_ref=None, compute_dtype: torch.dtype = torch.float16, use_cache: bool = False, **kwargs):
    """
    ...
    use_local = False
    if os.path.isdir(str(model_ref)) or (
        os.path.exists(str(model_ref)) and os.path.isfile(os.path.join(str(model_ref), "config.json"))
    ):
    ...
    transformers.models.qwen2_5_vl.modeling_qwen2_5_vl.Qwen2_5_VLForConditionalGeneration.forward = (
        qwen2_5_mixed_modality_forward_lantern
    )
    ...
    try:
        ...
        try:
            ...
            local_qwen_path = snapshot_download("Qwen/Qwen2.5-VL-3B-Instruct", local_files_only=True)
        except Exception:
            local_qwen_path = "Qwen/Qwen2.5-VL-3B-Instruct"
        proc_kwargs = {k: v for k, v in kwargs.items() if k != "local_files_only"}
        processor = AutoProcessor.from_pretrained(
            local_qwen_path, min_pixels=min_pixels, max_pixels=max_pixels, local_files_only=True, **proc_kwargs
        )
