...
def parse_output(x: str):
    ...
    outputs = {"pre_visual_text_think": None, "post_visual_text_think": [], "text_think": None, "answer": None}
    ...
    for pre_visual_text, post_visual_text in zip(outputs["pre_visual_text_think"], outputs["post_visual_text_think"]):
        ...
        if similarity_score_value > 60:
            print(
                f"\033[91mSimilarity score between {pre_visual_text} and {post_visual_text} is {similarity_score}\033[0m"
            )
...
if __name__ == "__main__":
    ...
    print(
        similarity_score(
            "girl in yellow shirt", "what about the girl in yellow shirt? asdjkloasjkldas das daosindas das"
        )
    )
