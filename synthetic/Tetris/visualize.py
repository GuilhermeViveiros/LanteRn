...
def make_grid(images, n_cols: int, padding: int = 6, bg=(12, 12, 16)) -> Image.Image:
    ...
    canvas = Image.new("RGB", (n_cols * cell_w + (n_cols + 1) * padding, n_rows * cell_h + (n_rows + 1) * padding), bg)
    for idx, im in enumerate(images):
        col = idx % n_cols
        row = idx // n_cols
        canvas.paste(im, (padding + col * (cell_w + padding), padding + row * (cell_h + padding)))
    return canvas
...
def parse_args():
    ...
    p.add_argument("--debug", action="store_true", help="Annotate with answer and transform description.")
    return p.parse_args()
